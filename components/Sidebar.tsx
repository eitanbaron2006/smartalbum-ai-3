...
import { AlbumSettings, FONTS, DesignSet } from '../types';
import { Upload, Settings, Image as ImageIcon, Layout, Printer, Palette, Eye, X, Beaker, Type, Download, Loader2, Sparkles, Bot, Plus, Trash2, Paperclip, Frame, Book, Save, LogOut } from 'lucide-react';
...
interface SidebarProps {
  ...
  onOpenDesignSetWizard: () => void;
  frontCoverUrl?: string;
  onSaveAlbum: () => void;
  onExitAlbum: () => void;
  savedDesignSets: DesignSet[];
  onApplyDesignSetFromLibrary: (setId: string) => void;
  onDeleteDesignSet: (setId: string) => void;
}
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  onOpenDesignSetWizard,
  frontCoverUrl,
  onSaveAlbum,
  onExitAlbum,
  savedDesignSets,
  onApplyDesignSetFromLibrary,
  onDeleteDesignSet,
}) => {
  ...
  const [activeTab, setActiveTab] = useState<'info' | 'style' | 'bg' | 'sets'>('style');
  ...
  const handleUseFrontCover = async () => {
    ...
    try {
      ...
      console.error("Failed to load front cover", e);
    }
  };
  ...
  // Explicit handlers to prevent default form behavior
      ...
      <div className="p-5 space-y-8 flex-1">
        ...
        {activeTab === 'sets' && (
          <section>
            ...
            {(savedDesignSets || []).length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-8">
                No saved sets. Create a design set and save it to your library!
              </p>
            ) : (
              <div className="grid grid-cols-2 gap-3 max-h-[60vh] overflow-y-auto pr-1">
                {(savedDesignSets || []).map(set => (
                  <div key={set.id} className="relative group/set aspect-[3/4]">
                    <button
                      type="button"
                      onClick={() => onApplyDesignSetFromLibrary(set.id)}
                      className="w-full h-full bg-gray-200 rounded-lg overflow-hidden border-2 border-transparent hover:border-indigo-500 transition-all"
                      title={`Apply "${set.name}"`}
                    >
                      {set.frontCoverUrl ? (
                        <img src={set.frontCoverUrl} alt={set.name} className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center bg-gray-100 text-gray-400">
                          <ImageIcon className="w-8 h-8" />
                        </div>
                      )}
                      <div className="absolute inset-0 bg-black/0 group-hover/set:bg-black/40 transition-all flex items-center justify-center text-white text-xs font-bold opacity-0 group-hover/set:opacity-100">
                        Apply
                      </div>
                    </button>
                    <div className="absolute bottom-1 left-1 right-1 p-1.5 bg-black/50 backdrop-blur-sm rounded-md text-white text-[10px] font-semibold truncate pointer-events-none">
                      {set.name}
                    </div>
                    <button
                      type="button"
                      onClick={() => onDeleteDesignSet(set.id)}
                      className="absolute top-1 right-1 p-1 bg-red-600/80 text-white rounded-full opacity-0 group-hover/set:opacity-100 hover:bg-red-500 transition-all"
                      title="Delete Set"
                    >
